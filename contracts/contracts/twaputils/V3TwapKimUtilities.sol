...
contract V3TwapKimUtilities is IV3TwapUtilities, Ownable {
    ...
    function getV3Pool(address _v3Factory, address _t0, address _t1) external view override returns (address) {
        ...
        PoolAddressKimMode.PoolKey memory _key = PoolAddressKimMode.PoolKey({token0: _token0, token1: _token1});
        return PoolAddressKimMode.computeAddress(IAlgebraFactory(_v3Factory).poolDeployer(), _key);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 _priceStableWETH9X96 = _adjustedPriceX96(
            ...
            _token0 == _WETH9 ? IAlgebraKimV3Pool(_nativeStablePool).token1() : _token0
        );
        if (_pricePool == _nativeStablePool) {
            return _priceStableWETH9X96;
        }
        uint256 _priceMainX96 = _adjustedPriceX96(IAlgebraKimV3Pool(_pricePool), _WETH9);
    ...
    function _adjustedPriceX96(IAlgebraKimV3Pool _pool, address _numeratorToken) internal view returns (uint256) {
